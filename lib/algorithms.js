...
exports.updateRecommendationsFor = function(userId, cb){
  ...
  userId = String(userId);
  console.log('++++ Starting updateRecommendationsFor function', userId);
  ...
  client.zrevrange(similarityZSet, 0, config.nearestNeighbors-1, function(err, mostSimilarUserIds){
    console.log('++++ Getting most similar user ids', mostSimilarUserIds);
    ...
    client.zrange(similarityZSet, 0, config.nearestNeighbors-1, function(err, leastSimilarUserIds){
      console.log('++++ Getting least similar user ids', leastSimilarUserIds);
      ...
      if (setsToUnion.length > 0){
        ...
        client.sunionstore(setsToUnion, function(err) {
          ...
          client.sdiff(tempAllLikedSet, Key.userLikedSet(userId), Key.userDislikedSet(userId), Key.userPassedSet(userId), function(err, notYetRatedItems){
            console.log('++++ getting list of recommended items that user has not yet rated', notYetRatedItems);
            ...
            async.each(notYetRatedItems,
              ...
              function(err){
                client.del(recommendedZSet, function(err){
                  console.log('++++ getting list of recommended items with scores', scoreMap);
                  async.each(scoreMap,
                    ...
                    function(err){
                      console.log('++++ items are written to recommended sorted set, deleting tempset');
