...
const removeStaleUsers = function(userIds) {
  const keys = [];
  const pipeline = client.pipeline();
  userIds.forEach(userId => {
    keys.push(Key.userLikedSet(userId));
    keys.push(Key.userDislikedSet(userId));
    keys.push(Key.userPassedSet(userId));
    keys.push(Key.recommendedZSet(userId));
    keys.push(Key.userIntersectionZSet(userId));
  });
  keys.forEach(key => {
    console.log(key);
    pipeline.del(key);
  ...
  return pipeline.exec().then(() => {
    console.log('Successfully deleted stale user keys');
  });
}

const removeStalePosts = function(postIds) {
  const keys = [];
  const pipeline = client.pipeline();
  postIds.forEach(postId => {
    keys.push(Key.itemLikedBySet(postId));
    keys.push(Key.itemDislikedBySet(postId));
  });
  keys.forEach(key => {
    console.log(key);
    pipeline.del(key);
  ...
  return pipeline.exec().then(() => {
    console.log('Successfully deleted stale post keys');
  });
}

const cleanUserKeys = function(userId, postIds) {
  const pipeline = client.pipeline();
  pipeline.srem(Key.userLikedSet(userId), postIds);
  pipeline.srem(Key.userDislikedSet(userId), postIds);
  pipeline.srem(Key.userPassedSet(userId), postIds);
  return pipeline.exec().then(() => {
    console.log(`Successfully cleaned user ${userId} keys`);
  });
}
...
const getHotness = function(likes, dislikes, date) {
  ...
  const score = likes*1.25 - dislikes*1.25;
