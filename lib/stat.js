  ...
  recommendFor: function(userId, numberOfRecs) {
    ...
    return new Promise((resolve, reject) => {
      ...
      */
      ...
      .then(results => {
        console.log('#### Resolving promise with Active items', results);
        resolve(results);
      })
      .catch(err => {
        /**
      ...
      .then(results => {
        console.log('#### Getting list of active items and recommended items', results);
      ...
      .then(results => {
        ...
        if (filteredZSetCreated) {
          return client.expire(Key.userFilteredActiveItemsZSet(userId), 600).then(() {
            resolve(results);
          });
        } else {
          resolve(results);
